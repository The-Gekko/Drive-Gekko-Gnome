...
arch_ver() { LC_ALL=C pacman -Si "$1" 2>/dev/null | awk -F': *' '/^Version/{print $2; exit}' || true; }
is_ver()   { [[ "$1" =~ ^[0-9][0-9A-Za-z.+_~]*$ ]]; }              # pkgver de pacman (sin '-' ni ':')
is_rel()   { [[ "$1" =~ ^[0-9]+(\.[0-9]+)?$ ]]; }
is_hex()   { [[ "$1" =~ ^[0-9a-f]+$ && ${#1} -eq $2 ]]; }
newer()    { [[ "$(vercmp "$1" "$2")" -gt 0 ]]; }                    # $1 mas nueva que $2
# escribe una variable simple (pkgver=, pkgrel=, _cvecommit=) SOLO en la linea
# que empieza por ella (los comentarios empiezan por '#', nunca por la variable)
...
if [[ -z "$full" ]]; then warn "gvfs: no se pudo leer la version en [extra] (¿pacman -Sy?); se omite"
  ...
  if [[ -z "$opts" ]]; then warn "gvfs $ext en [extra] pero no pude leer su meson_options.txt para confirmar la opcion google; no se bumpea"
...
if [[ -z "$full" ]]; then warn "gnome-online-accounts: no se pudo leer [extra]; se omite"
...
elif ! is_ver "$ext_v" || ! is_rel "$ext_r"; then warn "gnome-online-accounts: version con formato inesperado en [extra]: '$full'; se omite"
  ...
  elif [[ -n "$opts" ]] && ! grep -q "^option('google_files'" <<<"$opts"; then warn "GOA $ext_v YA NO TIENE la opcion 'google_files': investigar antes de subir"
  else
